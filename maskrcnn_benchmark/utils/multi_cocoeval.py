...
class COCOeval:
    ...
    def computeIoU(self, imgId, catId):
        ...
        # ious是(m, n), m是d的个数，即模型的预测检测框个数，n是g的框个数
    ...
    def evaluateImg(self, imgId, catId, aRng, maxDet):
        ...
        material_gtm = np.zeros((T, G))
        material_dtm = np.zeros((T, D))
        ...
        if not len(ious)==0:
            for tind, t in enumerate(p.iouThrs):
                for dind, d in enumerate(dt):
                    ...
                    if d['material_id'] == gt[m]['material_id']:
                        material_dtm[tind,dind]  = gt[m]['id']
                        material_gtm[tind,m]     = d['id']

                    # gtm[T, G]：gt匹配的dt的id
                    # dtm[T, D]：dt匹配的gt的id
                    # gtIg[G]：gt的_ignore标签
                    # dtIg[T, D]：dt匹配的gt的_ignore标签
        ...
        return {
                ...
                'dtMatches':    dtm,
                'gtMatches':    gtm,
                'material_dtMatches': material_dtm,
                'material_gtMatches': material_gtm,
                'dtScores':     [d['score'] for d in dt],
                'material_dtScores': [d['material_score'] for d in dt],
                'gtIgnore':     gtIg,
                'dtIgnore':     dtIg,
            }
    ...
    def accumulate(self, p = None):
        ...
        # A：区域范围，4
        # M：最大det数量，3
        # precision：TxRxKxAxM，-1
        # 代表缺失类别。记录各个recall阈值的precision值。
        ...
        material_precision = -np.ones((T, R, K, A, M))  # -1 for the precision of absent categories
        material_recall = -np.ones((T, K, A, M))
        material_scores = -np.ones((T, R, K, A, M))
        ...
        for k, k0 in enumerate(k_list):
            ...
            for a, a0 in enumerate(a_list):
                ...
                for m, maxDet in enumerate(m_list):
                    # 由于 evalImgs 的存放顺序是：按照K，A，I（imgId）
                    # E 相当于取出了所有 Image 在当前 k0，当前 a0 下的 evaluate 结果
                    E = [self.evalImgs[Nk + Na + i] for i in i_list]
                    E = [e for e in E if not e is None]
                    if len(E) == 0:
                        continue
                    dtScores = np.concatenate([e['dtScores'][0:maxDet] for e in E])
                    material_dtScores = np.concatenate([e['material_dtScores'][0:maxDet] for e in E])

                    # different sorting method generates slightly different results.
                    # mergesort is used to be consistent as Matlab implementation.
                    inds = np.argsort(-dtScores, kind='mergesort')
                    dtScoresSorted = dtScores[inds]
                    material_dtScoresSorted = material_dtScores[inds]

                    dtm  = np.concatenate([e['dtMatches'][:,0:maxDet] for e in E], axis=1)[:,inds]

                    material_dtm = np.concatenate([e['material_dtMatches'][:, 0:maxDet] for e in E], axis=1)[:, inds]
                    ...
                    material_tps = np.logical_and(material_dtm, np.logical_not(dtIg))
                    material_fps = np.logical_and(np.logical_not(material_dtm), np.logical_not(dtIg))

                    material_tp_sum = np.cumsum(material_tps, axis=1).astype(dtype=np.float)
                    material_fp_sum = np.cumsum(material_fps, axis=1).astype(dtype=np.float)

                    for t, (tp, fp) in enumerate(zip(material_tp_sum, material_fp_sum)):
                        tp = np.array(tp)
                        fp = np.array(fp)
                        nd = len(tp)
                        rc = tp / npig
                        ...
                        if nd:
                            material_recall[t,k,a,m] = rc[-1]
                        else:
                            material_recall[t,k,a,m] = 0
        ...
        self.eval = {
            ...
            'recall':   recall,
            'scores': scores,
            'material_precision': material_precision,
    ...
    def summarize(self):
        ...
        '''
        def _summarize( ap=1, iouThr=None, areaRng='all', maxDets=100, material_flag=False):
            p = self.params
            ...
            if material_flag:
                precision_name = 'material_precision'
                recall_name = 'material_recall'
            else:
